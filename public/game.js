...
function defaultState() {
  return {
    ...
    hintUsed: false, hintTrait: null, hintValue: null,
    victoryData: null,
    // Game 2
    g2started: false,
    g2guesses: [], g2won: false,
    g2hintUsed: false, g2hintValue: null,
    g2victoryName: null,
    // Game 3
    g3started: false,
    g3guesses: [], g3won: false,
    g3hintUsed: false, g3hintValue: null,
    g3victoryName: null,
  };
}
...
function saveState() {
  try {
    localStorage.setItem(TODAY_KEY, JSON.stringify({
      ...
      g3hintUsed, g3hintValue, g3victoryName,
    }));
  } catch {}
}
...
input2.addEventListener('keydown', e => {
  ...
  if (e.key === 'ArrowDown') { e.preventDefault(); activeSuggestion2 = Math.min(activeSuggestion2 + 1, items.length - 1); updateActiveSugg(items, activeSuggestion2, input2); }
  else if (e.key === 'ArrowUp') { e.preventDefault(); activeSuggestion2 = Math.max(activeSuggestion2 - 1, -1); updateActiveSugg(items, activeSuggestion2, input2); }
  else if (e.key === 'Enter') { if (activeSuggestion2 >= 0 && items[activeSuggestion2]) { e.preventDefault(); selectSuggestion(items[activeSuggestion2].textContent, input2, suggestBox2); } else { submitGuess2(); } }
...
input3.addEventListener('keydown', e => {
  ...
  if (e.key === 'ArrowDown') { e.preventDefault(); activeSuggestion3 = Math.min(activeSuggestion3 + 1, items.length - 1); updateActiveSugg(items, activeSuggestion3, input3); }
  else if (e.key === 'ArrowUp') { e.preventDefault(); activeSuggestion3 = Math.max(activeSuggestion3 - 1, -1); updateActiveSugg(items, activeSuggestion3, input3); }
  else if (e.key === 'Enter') { if (activeSuggestion3 >= 0 && items[activeSuggestion3]) { e.preventDefault(); selectSuggestion(items[activeSuggestion3].textContent, input3, suggestBox3); } else { submitGuess3(); } }
  else if (e.key === 'Escape') { hideSuggestions(suggestBox3); }
});
...
async function submitGuess() {
  ...
  if (data.correct) {
    won = true;
    victoryData = { name: data.cookie_name, skill: data.skill_name || '', cooldown: data.skill_cooldown || 0 };
    saveState();
    input.disabled     = true;
    submitBtn.disabled = true;
...
async function submitGuess2() {
  ...
  const cookie = COOKIES.find(c => c.cookie_name.toLowerCase() === raw.toLowerCase());
  if (!cookie) { showToast('Cookie not found — check your spelling!'); return; }
  if (g2guesses.includes(cookie.cookie_name)) { alreadyEl2.textContent = `Already guessed ${cookie.cookie_name}!`; return; }
  ...
  try {
    ...
    showToast('Connection error — please try again.');
    input2.disabled = false; submitBtn2.disabled = false;
    return;
  }
...
// GAME 2 — META & HINT
// ─────────────────────────────────────────
function g2wrongCount() { return g2won ? g2guesses.length - 1 : g2guesses.length; }

function updateMeta2() {
  if (!g2guesses.length) { metaEl2.textContent = ''; return; }
  metaEl2.textContent = g2guesses.length === 1 ? '1 guess so far' : `${g2guesses.length} guesses so far`;
}

function renderHint2(el, hintData) {
  el.textContent = '';
  const b1 = document.createElement('strong'); b1.textContent = hintData.rarity;
  const b2 = document.createElement('strong'); b2.textContent = hintData.type;
  const b3 = document.createElement('strong'); b3.textContent = hintData.position;
  el.append('💡 Rarity: ', b1, ' · Type: ', b2, ' · Position: ', b3);
}

function updateHint2() {
  if (g2won) { hintSection2.classList.remove('show'); return; }
...
async function submitGuess3() {
  ...
  const cookie = COOKIES.find(c => c.cookie_name.toLowerCase() === raw.toLowerCase());
  if (!cookie) { showToast('Cookie not found — check your spelling!'); return; }
  if (g3guesses.includes(cookie.cookie_name)) { alreadyEl3.textContent = `Already guessed ${cookie.cookie_name}!`; return; }
...
// GAME 3 — META & HINT
// ─────────────────────────────────────────
function g3wrongCount() { return g3won ? g3guesses.length - 1 : g3guesses.length; }

function updateMeta3() {
  if (!g3guesses.length) { metaEl3.textContent = ''; return; }
  metaEl3.textContent = g3guesses.length === 1 ? '1 guess so far' : `${g3guesses.length} guesses so far`;
}

function renderHint3(el, hintData) {
  el.textContent = '';
  const b1 = document.createElement('strong'); b1.textContent = hintData.primary_color;
  const b2 = document.createElement('strong'); b2.textContent = hintData.type;
  const b3 = document.createElement('strong'); b3.textContent = hintData.rarity;
  el.append('💡 Primary Color: ', b1, ' · Type: ', b2, ' · Rarity: ', b3);
}

function updateHint3() {
  if (g3won) { hintSection3.classList.remove('show'); return; }
...
function defaultStats() {
  return { currentStreak: 0, bestStreak: 0, totalPlayed: 0, totalWon: 0, totalGuesses: 0, lastCompleted: null };
}
...
function renderStats() {
  ...
  document.getElementById('statWinRate').textContent    = s.totalPlayed > 0 ? Math.round((s.totalWon / s.totalPlayed) * 100) + '%' : '0%';
